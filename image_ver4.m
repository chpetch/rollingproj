...
for frun = 1:pm.nof
    ...
    for i = 1:1:pm.nFrames
        ...
        procpic = imtophat(procpic, strel('disk', 10));
        se = strel(ones(3,3));
        basic_gradient = imdilate(procpic, se) - imerode(procpic, se);
        procpic = imfill(basic_gradient);
        procpic = procpic*(65535/double(max(procpic(:))));
        CC = bwconncomp(im2bw(procpic), 8);
        obj = regionprops(im2bw(procpic),'Area','centroid','EquivDiameter','ConvexArea','MajorAxisLength','MinorAxisLength');
...
%         %determining area
        if isempty(find(aa>100)) ~= 1 || isempty(find(aa<20)) ~= 1  
            aa(find(aa<20)) = [];
            aa(find(aa>100)) = [];
            center(find(aa<20),:) = [];
            center(find(aa>100),:) = [];
