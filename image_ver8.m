...
for frun = 1:pm.nof
    ...
    clf
    %pause
    clearvars -except pic pm.upbd pm.lwbd pm.tbf pm.nFrames zproject npic pm handles fldop listoftif frun fldname result
    ...
    [result,ntf,sframe] = constrainsforcells(t_cell,result,sframe,pm);
    ...
    if isempty(t_cell) ~= 1
        ...
        sframe(htd) = [];
        for i = 1:size(result.cpos_new,2)
            ntf(i) = size(result.cpos_new{i},1);
        end
        clear cvel cvelxy
...
if fldop == 1
    acv = get_acv(result.vidvel,result.ntf);
